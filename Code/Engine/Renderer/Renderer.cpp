...
{
	Mat44	ModelToWorldTransform = Mat44();
...
void Renderer::Startup()
{
	...
	Image defaultDiffuseTextureImage	= Image(IntVec2(1, 1), Rgba8::WHITE);
	Image defaultNormalTextureImage		= Image(IntVec2(1, 1), Rgba8(127, 127, 255));
	Image defaultSGETextureImage		= Image(IntVec2(1, 1), Rgba8(127, 127, 0));

	m_defaultDiffuseTexture = CreateTextureFromImage(defaultDiffuseTextureImage);
	m_defaultDiffuseTexture->m_name = "Default Diffuse";

	m_defaultNormalTexture = CreateTextureFromImage(defaultNormalTextureImage);
	m_defaultNormalTexture->m_name = "Default Normal";

	m_defaultSGETexture = CreateTextureFromImage(defaultSGETextureImage);
	m_defaultSGETexture->m_name = "Default SGE";

	BindTexture(m_defaultDiffuseTexture, 0);
	BindTexture(m_defaultNormalTexture, 1);
	BindTexture(m_defaultSGETexture, 2);
...
void Renderer::Shutdown()
{
	...
	m_defaultDiffuseTexture = nullptr;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void Renderer::DrawWireframeVertexArray(int numVertexes, const Vertex_PCU* vertexes)
{
	unsigned int vboSize = numVertexes * sizeof(Vertex_PCU);

	CopyCPUToGPU(vertexes, vboSize, m_immediateVBO);
	DrawWireframeVertexBuffer(m_immediateVBO, numVertexes);
}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void Renderer::DrawWireframeVertexArray(std::vector<Vertex_PCU> const& vertexes)
{
	DrawWireframeVertexArray(static_cast<int>(vertexes.size()), vertexes.data());
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void Renderer::DrawWireframeVertexBuffer(VertexBuffer* vbo, unsigned int vertexCount)
{
	SetStatesIfChanged();
	BindVertexBuffer(vbo);
...
void Renderer::CreateSamplerStates()
{
	...
	samplerDesc.MaxLOD = D3D11_FLOAT32_MAX;
	hr = m_device->CreateSamplerState(&samplerDesc, &m_samplerStates[static_cast<int>(SamplerMode::POINT_WRAP)]);
	if(!SUCCEEDED(hr))
	{
	...
	samplerDesc.ComparisonFunc = D3D11_COMPARISON_NEVER;
	samplerDesc.MaxLOD = D3D11_FLOAT32_MAX;
	hr = m_device->CreateSamplerState(&samplerDesc, &m_samplerStates[static_cast<int>(SamplerMode::POINT_MIRROR)]);
	if(!SUCCEEDED(hr))
	{
		ERROR_AND_DIE("CreateSamplerState for SamplerMode::POINT_MIRROR failed");
	}
	...
	samplerDesc.ComparisonFunc = D3D11_COMPARISON_NEVER;
	samplerDesc.MaxLOD = D3D11_FLOAT32_MAX;
	hr = m_device->CreateSamplerState(&samplerDesc, &m_samplerStates[static_cast<int>(SamplerMode::BILINEAR_MIRROR)]);
	if(!SUCCEEDED(hr))
	{
		ERROR_AND_DIE("CreateSamplerState for SamplerMode::BILINEAR_MIRROR failed");
	}
	...
	samplerDesc.ComparisonFunc = D3D11_COMPARISON_NEVER;
	samplerDesc.MaxLOD = D3D11_FLOAT32_MAX;
	hr = m_device->CreateSamplerState(&samplerDesc, &m_samplerStates[static_cast<int>(SamplerMode::TRILINEAR_CLAMP)]);
	if(!SUCCEEDED(hr))
	{
		ERROR_AND_DIE("CreateSamplerState for SamplerMode::TRILINEAR_CLAMP failed");
	}
	...
	samplerDesc.ComparisonFunc = D3D11_COMPARISON_NEVER;
	samplerDesc.MaxLOD = D3D11_FLOAT32_MAX;
	hr = m_device->CreateSamplerState(&samplerDesc, &m_samplerStates[static_cast<int>(SamplerMode::TRILINEAR_MIRROR)]);
	if(!SUCCEEDED(hr))
	{
		ERROR_AND_DIE("CreateSamplerState for SamplerMode::TRILINEAR_MIRROR failed");
	}
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void Renderer::SetModelConstants(Mat44 const& modelToWorldTransform, Rgba8 const& modelColor, int specialInt)
{
	ModelConstants modelConsts;
	...
	modelConsts.SpecialInt		= specialInt;
	CopyCPUToGPU(&modelConsts, sizeof(ModelConstants), m_modelCBO);
	BindConstantBuffer(k_modelConstantsSlot, m_modelCBO);
}
...
//------------------------------------------------------------------------------------------------------------------
Texture* Renderer::CreateOrGetTextureFromFilePath(char const* imageFilePath,int mipLevels)
{
	...
	if(textureImage.GetRawData() == nullptr)
	{
		DebuggerPrintf(Stringf("------------------------Image %s to create texture does not exist------------------------", imageFilePath).c_str());
		return nullptr;
	}

	Texture* newTexture = CreateTextureFromImage(textureImage, mipLevels);
	return newTexture;
}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
Texture* Renderer::CreateOrGetTextureFromFileNameAndType(std::string const& imageName, TextureType textureType, int mipLevels)
{

	std::string textureName = Stringf("%s", imageName.c_str());
	...
	switch(textureType)
	{
		...
		{
			textureName = Stringf("%s_a", imageName.c_str());
			break;
		}
		case TextureType::METAL:
		{
			textureName = Stringf("%s_m", imageName.c_str());
			break;
		}
		case TextureType::ROUGH:
		{
			textureName = Stringf("%s_r", imageName.c_str());
			break;
		}
		case TextureType::AO:
		{
			textureName = Stringf("%s_ao", imageName.c_str());
			break;
		}
	...
	if(textureImage.GetRawData() == nullptr)
	{
		DebuggerPrintf(Stringf("------------------------Image %s to create texture does not exist------------------------", imageFilePath.c_str()).c_str());
		return nullptr;
	}
...
//------------------------------------------------------------------------------------------------------------------
Texture* Renderer::CreateTextureFromImage(Image& textureImage, int mipLevels)
{
	HRESULT hr;
	...
	m_deviceContext->UpdateSubresource(newTexture->m_texture, 0, nullptr, textureImage.GetRawData(), textureData.SysMemPitch, 0);

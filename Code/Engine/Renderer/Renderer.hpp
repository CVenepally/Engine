...
public:
	...
	void						DrawWireframeVertexArray(int numVertexes, const Vertex_PCU* vertexes);
	void						DrawWireframeVertexArray(std::vector<Vertex_PCU> const& vertexes);
	...
	void						DrawWireframeVertexBuffer(VertexBuffer* vbo, unsigned int vertexCount);
	...
	void						SetModelConstants(Mat44 const& modelToWorldTransform = Mat44(), Rgba8 const& modelColor = Rgba8::WHITE, int specialInt = 0);
	...
	Texture*					CreateOrGetTextureFromFilePath(char const* imageFilePath, int mipLevels = 1);
	Texture*					CreateOrGetTextureFromFileNameAndType(std::string const& imageName, TextureType textureType = TextureType::DIFFUSE, int mipLevels = 1);
	Texture*					GetTextureForFileName(char const* imageFilePath);
	Texture*					CreateTextureFromImage(Image& textureImage, int mipLevels = 1);
...
private:
	...
	Texture*					m_defaultSGETexture			= nullptr;

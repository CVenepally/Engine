...
uchar const KEYCODE_TAB              =VK_TAB;
...
// 	m_cursorState.m_cursorClientPosition = IntVec2(windowsCursorClientPosition.x, windowsCursorClientPosition.y);
	...
	{
		if(m_keyStates[keyIndex].m_isPressed)
		{
			m_keyStates[keyIndex].m_holdSeconds += Clock::GetSystemClock().GetDeltaSeconds();
		}
	}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool InputSystem::WasKeyHeldForSeconds(unsigned char keyCode, float seconds)
{
	bool result = m_keyStates[keyCode].m_holdSeconds >= seconds;
	if(result)
	{
		m_keyStates[keyCode].m_holdSeconds = 0.f;
	}

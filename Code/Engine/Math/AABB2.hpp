...
public:
	...
	void ShrinkAABB2(float amountToShrinkBy);
	void AddPaddingOnLeft(float padding);
	void AddPaddingOnRight(float padding);
	void AddPaddingOnTop(float padding);
	void AddPaddingOnBottom(float padding);

	AABB2 GetBoxFromUVs(AABB2 boxUVs) const;
	AABB2 GetBoxFromUVs(float minxUV, float minyUV, float maxxUV, float maxyUV) const;

...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vec3::Vec3(Vec2 const& vec2, float inZ)
	: x(vec2.x)
	, y(vec2.y)
	, z(inZ)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vec3::Vec3(int initialXYZ)
	: x(static_cast<float>(initialXYZ))
	, y(static_cast<float>(initialXYZ))
	, z(static_cast<float>(initialXYZ))
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vec3::Vec3(IntVec2 const& intVec2, int inZ)
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vec3::Vec3(IntVec2 const& intVec2, float inZ)
	: x(static_cast<float>(intVec2.x))
	, y(static_cast<float>(intVec2.y))
	, z(inZ)
{}
...
//------------------------------------------------------------------------------------------------------------------
Vec3::Vec3(IntVec3 const& intVec3)
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vec3::Vec3(std::vector<float> const& floatArray)
	: Vec3(0.f, 0.f, 0.f)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vec3::Vec3(std::vector<double> const& floatArray)
	: Vec3(0.f, 0.f, 0.f)
{
	if(floatArray.size() >= 3)
	{
		x = static_cast<float>(floatArray[0]);
		y = static_cast<float>(floatArray[1]);
		z = static_cast<float>(floatArray[2]);
	}
}

Vec3::Vec3(float* floatArray)
	: Vec3(floatArray[0], floatArray[1], floatArray[2])
{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vec3 const Vec3::MakeFromString(char const* textVec3, char delimiter)
{
	Vec3 vec3FromString = Vec3();

	vec3FromString.SetFromText(textVec3, delimiter);

	return vec3FromString;
}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vec3 const Vec3::MakeFromWord(std::string const& vec3Word)
{
	if(vec3Word == "forward" || vec3Word == "Forward")
	{
		return Vec3::FORWARD;
	}
	else if(vec3Word == "back" || vec3Word == "Back")
	{
		return Vec3::BACKWARD;
	}
	else if(vec3Word == "left" || vec3Word == "Left")
	{
		return Vec3::LEFT;
	}
	else if(vec3Word == "right" || vec3Word == "Right")
	{
		return Vec3::RIGHT;
	}
	else if(vec3Word == "Up" || vec3Word == "up")
	{
		return Vec3::UP;
	}
	else if(vec3Word == "down" || vec3Word == "Down")
	{
		return Vec3::DOWN;
	}
	return Vec3();
...
void Vec3::GetAsFloatArray(float* out_array) const
{
	out_array[0] = x;
	out_array[1] = x;
	out_array[2] = x;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
IntVec3 Vec3::GetAsIntVec3() const
{
...
//------------------------------------------------------------------------------------------------------------------
void Vec3::SetFromText(char const* textVec3, char delimiter)
{

	Strings vec3Strings = SplitStringOnDelimiter(textVec3, delimiter);

...
//------------------------------------------------------------------------------------------------------------------
RaycastResult2D::RaycastResult2D(bool impactResult, Vec2 const& rayStartPos, Vec2 const& rayFwdNormal, float rayLength)
...
//------------------------------------------------------------------------------------------------------------------
RaycastResult2D::RaycastResult2D(bool impactResult, float impactDistance, Vec2 const& impactPos, Vec2 const& impactNormal, Vec2 const& rayStartPos, Vec2 const& rayFwdNormal, float rayLength)
	: m_didImpact(impactResult)
...
RaycastResult2D RaycastVsPlane2D(Vec2 const& startPos, Vec2 const& fwdNormal, float maxDist, Plane2 const& plane)
{
	...
	rayResult.m_rayForwardNormal	= fwdNormal;
	...
	float altitudeFromEnd = plane.GetAltitudeFromPoint(startPos + (maxDist * fwdNormal));

	if(altitudeFromStart * altitudeFromEnd > 0)
	{
		return rayResult;
	}

	float forwardProjectedOnPlaneNormal = DotProduct2D(fwdNormal, plane.m_normal);

	rayResult.m_impactDistance = altitudeFromStart / (-forwardProjectedOnPlaneNormal);
	rayResult.m_impactPos = startPos + fwdNormal * rayResult.m_impactDistance;
	rayResult.m_didImpact = true;
	rayResult.m_impactNormal = altitudeFromStart < 0.f ? -plane.m_normal : plane.m_normal;
...
RaycastResult2D RaycastVsConvexHull2D(Vec2 const& startPos, Vec2 const& fwdNormal, float maxDist, ConvexHull2 const& hull)
{
	...
	rayResult.m_rayMaxLength = maxDist;
	rayResult.m_didImpact = false;
	rayResult.m_impactDistance = FLT_MAX;

	Vec2 farthestEntryPoint = startPos;
	Vec2 closestExitPoint = startPos + fwdNormal * maxDist;

	float farthestEntryDistance = 0.f;
	float closestExitDistance = maxDist;

	Plane2 entryPlane;

	for(Plane2 const& plane : hull.m_planes)
	{
		RaycastResult2D result = RaycastVsPlane2D(startPos, fwdNormal, maxDist, plane);

		if(result.m_didImpact)
		{
			if(DotProduct2D(plane.m_normal, fwdNormal) > 0 && result.m_impactDistance < closestExitDistance) // Exit
			{
				closestExitDistance = result.m_impactDistance;
				closestExitPoint = result.m_impactPos;
			}
			else if(DotProduct2D(plane.m_normal, fwdNormal) < 0 && result.m_impactDistance > farthestEntryDistance) // Entry
			{
				farthestEntryDistance = result.m_impactDistance;
				farthestEntryPoint = result.m_impactPos;
				entryPlane = plane;
			}
		}
	}
	
	if(farthestEntryDistance > closestExitDistance)
	{
		return rayResult;
	}

	Vec2 midPoint = (farthestEntryPoint + closestExitPoint) * 0.5f;

	if(!IsPointInsideConvexHull2(midPoint, hull))
	{
		return rayResult;
	}

	rayResult.m_didImpact		= true;
	rayResult.m_impactNormal	= entryPlane.m_normal;
	rayResult.m_impactDistance	= farthestEntryDistance;
	rayResult.m_impactPos		= farthestEntryPoint;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
RaycastResult3D::RaycastResult3D(bool impactResult, Vec3 const& rayStartPos, Vec3 const& rayFwdNormal, float rayLength)
	: m_didImpact(impactResult)
	, m_rayStartPos(rayStartPos)
	, m_rayForwardNormal(rayFwdNormal)
	, m_rayMaxLength(rayLength)
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
RaycastResult3D::RaycastResult3D(bool impactResult, float impactDistance, Vec3 const& impactPos, Vec3 const& impactNormal, Vec3 const& rayStartPos, Vec3 const& rayFwdNormal, float rayLength)
	: m_didImpact(impactResult)
	, m_impactDistance(impactDistance)
	, m_impactNormal(impactNormal)
	, m_rayStartPos(rayStartPos)

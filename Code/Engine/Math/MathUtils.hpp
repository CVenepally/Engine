...
// Used to modify the value so that it is a multiple of alignment >= value (13 aligned up to 8 is 16, 18 aligned up to 8 is 24)
template <typename T>
...
// Used to modify the value so that it is a multiple of alignment <= value (13 aligned down to 8 is 8, 18 aligned up to 8 is 16)
template <typename T>
...
template<typename T>
T GetMax(T numA, T numB)
{
	return (numA >= numB) ? numA : numB;
}

template<typename T>
T GetMin(T numA, T numB)
{
	return (numA <= numB) ? numA : numB;
}
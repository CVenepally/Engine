...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void AABB2::ShrinkAABB2(float amountToShrinkBy)
{
	float halfAmountToShrinkBy = amountToShrinkBy / 2.f;

	float width = m_maxs.x - m_mins.x;
	float height = m_maxs.y - m_mins.y;

	m_mins.x += width * (halfAmountToShrinkBy);
	m_mins.y += height * (halfAmountToShrinkBy);
	m_maxs.x -= width * ( halfAmountToShrinkBy);
	m_maxs.y -= height * (halfAmountToShrinkBy);
}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void AABB2::AddPaddingOnLeft(float padding)
{
	m_mins.x += padding;
}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void AABB2::AddPaddingOnRight(float padding)
{
	m_maxs.x -= padding;
}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void AABB2::AddPaddingOnTop(float padding)
{
	m_maxs.y -= padding;
}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void AABB2::AddPaddingOnBottom(float padding)
{
	m_mins.y += padding;
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
AABB2 AABB2::GetBoxFromUVs(float minxUV, float minyUV, float maxxUV, float maxyUV) const
{
	return 	GetBoxFromUVs(AABB2(Vec2(minxUV, minyUV), Vec2(maxxUV, maxyUV)));
}

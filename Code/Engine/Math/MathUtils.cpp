...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
uint8_t GetClamped(uint8_t value, uint8_t minValue, uint8_t maxValue)
{
	if(value >= minValue && value <= maxValue)
	{
		return value;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
int8_t GetClamped(int8_t value, int8_t minValue, int8_t maxValue)
{
	if(value >= minValue && value <= maxValue)
	{
		return value;
...
float GetFractionWithinRange(float value, float rangeStart, float rangeEnd)
{
    ...
    float fraction = valueStartDiff / (range + 0.00001f);
...
float GetVectorDistance2D(IntVec2 const& intVecA, IntVec2 const& intVecB)
{
    float deltaX = static_cast<float>(intVecB.x - intVecA.x);
    float deltaY = static_cast<float>(intVecB.y - intVecA.y); 
...
int GetVectorDistanceSquared2D(IntVec2 const& intVecA, IntVec2 const& intVecB)
{
    int deltaX = intVecB.x - intVecA.x;
    int deltaY = intVecB.y - intVecA.y;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool IsPointBehindPlane2(Vec2 const& referencePoint, Plane2 const& plane)
{
    return plane.GetAltitudeFromPoint(referencePoint) < 0.f;
}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool IsPointInsideConvexHull2(Vec2 const& refPoint, ConvexHull2 const& hull)
{
    for(Plane2 const& plane: hull.m_planes)
    {
        if(!IsPointBehindPlane2(refPoint, plane)) 
            return false;
    }
...
//------------------------------------------------------------------------------------------------------------------
bool IsAligned(int value, int alignment)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
unsigned int GetMin(unsigned int numA, unsigned int numB)
{
	unsigned int num = numA <= numB ? numA : numB;

	return num;
}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
unsigned int GetMax(unsigned int numA, unsigned int numB)
{
	unsigned int num = numA >= numB ? numA : numB;

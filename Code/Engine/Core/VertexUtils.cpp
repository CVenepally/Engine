...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void TransformVertexArray3D(std::vector<Vertex_PCU>& verts, Mat44 const& transform, float scale)
{
	for(int vertex = 0; vertex < static_cast<int>(verts.size()); ++vertex)
	{
		Vec3& position = verts[vertex].m_position;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void TransformVertexArray3D(std::vector<Vertex_PCUTBN>& verts, Mat44 const& transform, float scale, bool transformTBN)
{
	for(int vertex = 0; vertex < static_cast<int>(verts.size()); ++vertex)
	{
		Vec3& position = verts[vertex].m_position;
		...
		if(transformTBN)
		{
			Vec3& tangent = verts[vertex].m_tangent;
			Vec3& bitangent = verts[vertex].m_bitangent;
			Vec3& normal = verts[vertex].m_normal;

			tangent = transform.TransformPosition3D(tangent);
			bitangent = transform.TransformPosition3D(bitangent);
			normal = transform.TransformPosition3D(normal);
		}
	}
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void AddVertsForRay2(std::vector<Vertex_PCU>& verts, Ray2 const& ray, float arrowSize, float lineThickness, Rgba8 const& color)
{
	AddVertsForRay2(verts, ray.m_startPos, ray.m_startPos + ray.m_forwardNormal*ray.m_maxLength, arrowSize, lineThickness, color);
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void AddVertsForQuad3DPerVertexColor(std::vector<Vertex_PCUTBN>& verts, std::vector<unsigned int>& indices, Vec3 const& bottomLeft, Vec3 const& bottomRight, Vec3 const& topRight, Vec3 const& topLeft, Rgba8 const& vertAColor, Rgba8 const& vertBColor, Rgba8 const& vertCColor, Rgba8 const& vertDColor, AABB2 const& UVs)
{
	Vec3 tangent = (bottomRight - bottomLeft).GetNormalized();
	Vec3 biTangent = (topLeft - bottomLeft).GetNormalized();
	...
	verts.push_back(Vertex_PCUTBN(bottomLeft, vertAColor, UVs.m_mins, tangent, biTangent, bottomLeftNormal));
	verts.push_back(Vertex_PCUTBN(bottomRight, vertBColor, Vec2(UVs.m_maxs.x, UVs.m_mins.y), tangent, biTangent, bottomRightNormal));
	verts.push_back(Vertex_PCUTBN(topRight, vertCColor, UVs.m_maxs, tangent, biTangent, topRightNormal));
	verts.push_back(Vertex_PCUTBN(topLeft, vertDColor, Vec2(UVs.m_mins.x, UVs.m_maxs.y), tangent, biTangent, topLeftNormal));
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void AddVertsForWireframeQuad3D(std::vector<Vertex_PCU>& verts, Vec3 const& bottomLeft, Vec3 const& bottomRight, Vec3 const& topRight, Vec3 const& topLeft, Rgba8 const& color, AABB2 const& UVs)
{	
	verts.push_back(Vertex_PCU(bottomLeft, color, UVs.m_mins));
	verts.push_back(Vertex_PCU(bottomRight, color, Vec2(UVs.m_maxs.x, UVs.m_mins.y)));
...
void AddVertsForAABB3D(std::vector<Vertex_PCU>& verts, AABB3 const& bounds, Rgba8 const& color, AABB2 const& UVs)
{
	...
	// Positive X Face
	AddVertsForQuad3D(verts, eightCornerPoints[AABB3_FORWARD_RIGHT_BOTTOM], eightCornerPoints[AABB3_FORWARD_LEFT_BOTTOM], eightCornerPoints[AABB3_FORWARD_LEFT_TOP], eightCornerPoints[AABB3_FORWARD_RIGHT_TOP], color, UVs);

	// Negative X Face
	AddVertsForQuad3D(verts, eightCornerPoints[AABB3_BACK_LEFT_BOTTOM], eightCornerPoints[AABB3_BACK_RIGHT_BOTTOM], eightCornerPoints[AABB3_BACK_RIGHT_TOP], eightCornerPoints[AABB3_BACK_LEFT_TOP], color, UVs);

	// Positive Y Face
	AddVertsForQuad3D(verts, eightCornerPoints[AABB3_FORWARD_LEFT_BOTTOM], eightCornerPoints[AABB3_BACK_LEFT_BOTTOM], eightCornerPoints[AABB3_BACK_LEFT_TOP], eightCornerPoints[AABB3_FORWARD_LEFT_TOP], color, UVs);

	// Negative Y Face
	AddVertsForQuad3D(verts, eightCornerPoints[AABB3_BACK_RIGHT_BOTTOM], eightCornerPoints[AABB3_FORWARD_RIGHT_BOTTOM], eightCornerPoints[AABB3_FORWARD_RIGHT_TOP], eightCornerPoints[AABB3_BACK_RIGHT_TOP], color, UVs);

	// Positive Z Face
	AddVertsForQuad3D(verts, eightCornerPoints[AABB3_BACK_RIGHT_TOP], eightCornerPoints[AABB3_FORWARD_RIGHT_TOP], eightCornerPoints[AABB3_FORWARD_LEFT_TOP], eightCornerPoints[AABB3_BACK_LEFT_TOP], color, UVs);

	// Negative Z Face
	AddVertsForQuad3D(verts, eightCornerPoints[AABB3_FORWARD_RIGHT_BOTTOM], eightCornerPoints[AABB3_BACK_RIGHT_BOTTOM], eightCornerPoints[AABB3_BACK_LEFT_BOTTOM], eightCornerPoints[AABB3_FORWARD_LEFT_BOTTOM], color, UVs);
}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void AddVertsForAABB3D(std::vector<Vertex_PCUTBN>& verts, std::vector<uint32_t>& indices, AABB3 const& bounds, Rgba8 const& color, AABB2 const& UVs)
{

	std::vector<Vec3> eightCornerPoints;
	bounds.GetCornerPoints(eightCornerPoints);

	AddVertsForQuad3D(verts, indices, eightCornerPoints[AABB3_FORWARD_RIGHT_BOTTOM], eightCornerPoints[AABB3_FORWARD_LEFT_BOTTOM], eightCornerPoints[AABB3_FORWARD_LEFT_TOP], eightCornerPoints[AABB3_FORWARD_RIGHT_TOP], color, UVs);

	// Negative X Face
	AddVertsForQuad3D(verts, indices, eightCornerPoints[AABB3_BACK_LEFT_BOTTOM], eightCornerPoints[AABB3_BACK_RIGHT_BOTTOM], eightCornerPoints[AABB3_BACK_RIGHT_TOP], eightCornerPoints[AABB3_BACK_LEFT_TOP], color, UVs);

	// Positive Y Face
	AddVertsForQuad3D(verts, indices, eightCornerPoints[AABB3_FORWARD_LEFT_BOTTOM], eightCornerPoints[AABB3_BACK_LEFT_BOTTOM], eightCornerPoints[AABB3_BACK_LEFT_TOP], eightCornerPoints[AABB3_FORWARD_LEFT_TOP], color, UVs);

	// Negative Y Face
	AddVertsForQuad3D(verts, indices, eightCornerPoints[AABB3_BACK_RIGHT_BOTTOM], eightCornerPoints[AABB3_FORWARD_RIGHT_BOTTOM], eightCornerPoints[AABB3_FORWARD_RIGHT_TOP], eightCornerPoints[AABB3_BACK_RIGHT_TOP], color, UVs);

	// Positive Z Face
	AddVertsForQuad3D(verts, indices, eightCornerPoints[AABB3_BACK_RIGHT_TOP], eightCornerPoints[AABB3_FORWARD_RIGHT_TOP], eightCornerPoints[AABB3_FORWARD_LEFT_TOP], eightCornerPoints[AABB3_BACK_LEFT_TOP], color, UVs);

	// Negative Z Face
	AddVertsForQuad3D(verts, indices, eightCornerPoints[AABB3_FORWARD_RIGHT_BOTTOM], eightCornerPoints[AABB3_BACK_RIGHT_BOTTOM], eightCornerPoints[AABB3_BACK_LEFT_BOTTOM], eightCornerPoints[AABB3_FORWARD_LEFT_BOTTOM], color, UVs);
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void AddVertsForConvexPoly2(std::vector<Vertex_PCU>& verts, std::vector<Vec2> positions, Rgba8 const& color)
{
	int finalIndex = static_cast<int>(positions.size()) - 1;
	int startIndex = 1;

	while(startIndex < finalIndex)
	{
		verts.push_back(Vertex_PCU(positions[0], color));
		verts.push_back(Vertex_PCU(positions[startIndex], color));
		verts.push_back(Vertex_PCU(positions[startIndex + 1], color));

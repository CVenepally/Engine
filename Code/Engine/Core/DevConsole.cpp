...
const Rgba8 DevConsole::SUCCESS			= Rgba8(0, 255, 50, 255);
...
void DevConsole::Startup()
{
	...
	g_eventSystem->SubscribeEventCallbackFunction("MouseWheel", DevConsole::OnMouseWheel);
	...
	g_eventSystem->SubscribeEventCallbackFunction("Echo", DevConsole::OnEchoEvent);
...
void DevConsole::Shutdown()
{
	...
	g_eventSystem->UnsubscribeEventCallbackFunction("MouseWheel", DevConsole::OnMouseWheel);
	...
	g_eventSystem->UnsubscribeEventCallbackFunction("Echo", DevConsole::OnEchoEvent);
}
...
void DevConsole::BeginFrame()
{
	...
	m_config.m_devConsoleCamera.SetOrthographicView(Vec2(0.f, 0.f), Vec2(g_theWindow->GetClientDimensions()));
...
void DevConsole::AddLine(Rgba8 const& color, std::string const& text)
{
	...
	m_lineRenderStartIndex = static_cast<int>(m_lines.size()) - 1;
}
...
void DevConsole::ToggleMode(DevConsoleMode mode)
{
	...
	if(m_mode == HIDDEN)
	{
		...
		if(m_lineRenderStartIndex < 0)
		{
			m_lineRenderStartIndex = 0;
		}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool DevConsole::OnMouseWheel(EventArgs& args)
{
	if(!g_devConsole)
	{
		return false;
	...
	float wheelDelta = args.GetValue("MouseWheel", 0.f);

	if(wheelDelta > 0.f)
	{
// 		int startIndexesToSkip = 1 + (static_cast<int>(wheelDelta) % 5);
		
		g_devConsole->m_lineRenderStartIndex -= 1;

		if(g_devConsole->m_lineRenderStartIndex <= 0)
			g_devConsole->m_lineRenderStartIndex = 0;
	...
	else if(wheelDelta < 0.f)
	{
		...
		if(g_devConsole->m_lineRenderStartIndex >= static_cast<int>(g_devConsole->m_lines.size()))
		{
			g_devConsole->m_lineRenderStartIndex = static_cast<int>(g_devConsole->m_lines.size());
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool DevConsole::OnEchoEvent(EventArgs& args)
{
	...
	Rgba8 color = args.GetValue("color", INFO_MINOR);


	g_devConsole->AddLine(color, message);
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void DevConsole::Render(AABB2 const& bounds, DX12Renderer* rendererOverride) const
{
	std::scoped_lock<std::recursive_mutex> lock(m_devConsoleMutex);
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
void DevConsole::Render_OpenFull(AABB2 const& bounds, DX12Renderer& renderer, BitmapFont& font, float fontAspect) const
{
	std::scoped_lock<std::recursive_mutex> lock(m_devConsoleMutex);
	...
	PipelineStateObject* tempPSO = nullptr;


	tempPSO = renderer.CreateOrGetPipelineStateObject(nullptr, nullptr, InputLayoutType::VERTEX_PCU, RasterizerMode::SOLID_CULL_NONE, BlendMode::ALPHA, DepthMode::READ_WRITE_LESS_EQUAL);
	renderer.SetPipelineState(tempPSO);
 	renderer.SetModelConstants();
	renderer.BindTexture(nullptr, RootParameters::DIFFUSE_TEXTURE_DT);
	...
	renderer.SetModelConstants();
	renderer.BindTexture(&font.GetTexture(), RootParameters::DIFFUSE_TEXTURE_DT);
...
void DevConsole::Render_OpenFull(AABB2 const& bounds, Renderer& renderer, BitmapFont& font, float fontAspect) const
{
	...
	// console text render
	std::vector<Vertex_PCU> textVerts;
	int linesToRender = static_cast<int>(GetClamped(static_cast<float>(numLinesToRender), 0.f, static_cast<float>(m_lineRenderStartIndex)));

	for(int i = 0; i < linesToRender; ++i)
	{
		DevConsoleLine const& currentLine = m_lines[m_lineRenderStartIndex - i];
